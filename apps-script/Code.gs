...
function doPost(e) {
  try {
    ...
    if (payload.faxNumber) {
      return responseHtml_({ ok: true, ignored: true });
    }
...
/**
 * Returns an HTML document rather than JSON so the website can submit using
 * a normal form targeted at a hidden iframe (no cross-origin fetch/CORS dependency).
 * The response notifies the parent page through postMessage.
 */
function responseHtml_(result) {
  ...
  const html = [
    ...
    '<script>',
    'window.parent.postMessage({type:"consultation-form-result",payload:' + safeJson + '},' + safeOrigin + ');',
    '</script>',
    '</body></html>'
  ].join('');
